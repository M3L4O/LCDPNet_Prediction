    ...
    def __getitem__(self, idx):
        ...
        if self.have_gt:
            ...
            if res_item[GT_FPATH].endswith(".hdr"):
                ...
                gt_img = cv2.imread(self.gt_list[idx], flags=cv2.IMREAD_ANYDEPTH)[
                    :, :, [2, 1, 0]
                ]
    ...
    def setup(self, stage):
        ...
        if opt[TEST_DATA] and opt[TEST_DATA][INPUT]:
            self.test_dataset = ImagesDataset(
                opt, ds_type=TEST_DATA, transform=self.test_transform, batchsize=1
            )
...
class ColorCorrectionDataset(Dataset):
    def __init__(
        ...
        self.images_id = glob(ph.join(image_dir, "*"))
    ...
    def __getitem__(self, index: int) -> tuple:
        ...
        image = cv2.resize(cv2.imread(image_id, cv2.IMREAD_COLOR), image_shape)
